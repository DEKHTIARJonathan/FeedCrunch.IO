...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `openedLinksCount`  AS  select `visits`.`ipAddress` AS `ipAddress`,count(0) AS `vcount` from `visits` where (`visits`.`webpage` like '/article.php?postID=___') group by `visits`.`ipAddress` order by count(0) desc ;
