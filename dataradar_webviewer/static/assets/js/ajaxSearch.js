...
$('#search-bar').keyup(function(){
	...
	request = $.ajax({
		...
		success: function(data) {
				...
				var i;
				var articles = rslt.articles;
				for (i = 0; i < articles.length; ++i) {
					var str_search = rslt.search_str;
					var title = highlight(articles[i]["title"], str_search);
					var domain_name = highlight(articles[i]["domain_name"], str_search);
					var published_date = articles[i]["when"];
